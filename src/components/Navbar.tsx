...
const Navbar = () => {
  ...
  return (
    ...
    >
      <nav className="container mx-auto px-4 md:px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center space-x-8">
              ...
              <Link
                ...
                className={`font-medium text-base transition-all duration-300 relative group ${
                  location.pathname === item.path
                    ? "text-bharat-600"
                    : "text-gray-700 hover:text-bharat-600"
                }`}
              >
                {item.name}
          ...
          {isAuthenticated ? (
            ...
            <Button 
              asChild
              className="bg-bharat-600 hover:bg-bharat-700 text-white shadow-button transition-all duration-300 hover:shadow-lg"
            >
      ...
      {/* Mobile menu */}
      {isOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white/95 backdrop-blur-lg shadow-md py-4 px-4 space-y-4 animate-fade-in">
          {navItems.map((item) => (
            <Link
              key={item.name}
              to={item.path}
          ...
          {!isAuthenticated && (
            <Button 
              asChild
              className="w-full mt-4 bg-bharat-600 hover:bg-bharat-700 text-white"
            >
              <Link to="/dashboard">Login</Link>
            </Button>
          )}
